...
class BookSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Book
        fields = ["id", "title", "author", "description", "image","is_available", "created_at", "is_borrowed_by_me"]
    ...
    def get_is_borrowed_by_me(self, obj):
        ...
        if request and request.user.is_authenticated:
            result = Borrowing.objects.filter(book=obj, user=request.user, returned_date__isnull=True).exists()
            print(f"Borrowing record for user {request.user} and book '{obj.title}': {result}")
            return result
        return False
