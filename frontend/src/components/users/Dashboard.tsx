...
export default function Dashboard() {
  ...
  function handleReturn(bookId: number) {
    setLoading(true);
    api
      .post(`/books/${bookId}/return_book/`, {}, { headers: getAuthHeader() })
      .then((response) => {
      ...
      .then((data) => {
        setProfile((prev) =>
          prev
            ? {
                ...prev,
                active_borrowings: prev.active_borrowings.filter(
                  (b) => b.book !== bookId
                ),
              }
            : null
        ...
        <Grid container spacing={3} sx={{ mt: 1 }}>
          {profile?.active_borrowings.map((checkout) => (
            <Grid key={checkout.id}>
              <Card
                sx={{
                  borderTop: checkout.is_overdue ? '4px solid red' : 'none',
                }}
              >
                <CardContent>
                  ...
                  <Typography variant="body2" color="textSecondary">
                    Borrowed:{' '}
                    {new Date(checkout.borrowed_date).toLocaleDateString()}
                  </Typography>
                  {checkout.is_overdue && (
                    <Chip
                      label="OVERDUE"
                ...
                <CardActions>
                  <Button
                    size="small"
                    onClick={() => handleReturn(checkout.book)}
                  >
                    Return Book
                  </Button>
                </CardActions>
